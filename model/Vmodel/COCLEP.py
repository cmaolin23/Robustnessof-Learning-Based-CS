...
class Contra(nn.Module):
    ...
    def forward(self, h, h_aug, tau, train, alpha, lam, edge_index):
        ...
        numerator1 = torch.mm(h[q], h.t())
        norm1 = torch.norm(h, dim=-1, keepdim=True)
        denominator1 = torch.mm(norm1[q], norm1.t())
        sim1_ = numerator1 / denominator1
        sim1 = torch.exp(sim1_ / tau)

        numerator_aug1 = torch.mm(h_aug[q], h_aug.t())
        norm_aug1 = torch.norm(h_aug, dim=-1, keepdim=True)
        denominator_aug1 = torch.mm(norm_aug1[q], norm_aug1.t())
        sim_aug1_ = numerator_aug1 / denominator_aug1
        sim_aug1 = torch.exp(sim_aug1_ / tau)

        numerator2 = torch.mm(h[q], h_aug.t())
        denominator2 = torch.mm(norm1[q], norm_aug1.t())
        sim2_ = numerator2 / denominator2
        sim2 = torch.exp(sim2_ / tau)

        numerator_aug2 = torch.mm(h_aug[q], h.t())
        denominator_aug2 = torch.mm(norm_aug1[q], norm1.t())
...
class COCLEP(nn.Module):
    ...
    def compute_loss(self, q,pos, edge_index,edge_index_aug,feats):
        loss = None
    ...
    def forward(self,  q,pos, edge_index,edge_index_aug,feats):
        ...
        loss,h_r = self.compute_loss(q,pos, edge_index,edge_index_aug,feats)
